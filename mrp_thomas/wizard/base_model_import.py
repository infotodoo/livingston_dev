...
class BaseModelImport(models.TransientModel):
    ...
    def action_import(self):
        ...
        this.write({
            'state': 'get',
            'filedata': out,
            'filename': '%s.xlsx' % self.model_id.model.replace('.','_')
        })
    ...
    def _import_data(self, source):
        ...
        for l in lis:
            try:
                ...
                if l.get('id'):
                    model_obj = False
                    id = self._field_id_modify(l.get('id'))
                    id_int = id if isinstance(id, int) else self.env.ref(id).id
                    model_obj = self.env[self.model_id.model].browse(id_int)
                    del l['id']
                    nl = self._field_type(l, self.model_id, 'write')
                    # Check field type
                    nl.update(import_date=fields.Datetime.now())
                    model_obj.write(nl)
                # Create
                else:
                    model_obj = self.env[self.model_id.model]
                    nl = self._field_type(l, self.model_id, 'create')
                    nl.update(import_date=fields.Datetime.now())
                    model_obj.create(nl)
                ...
                for j, k in l.items():
                    ...
                    count += 1
                worksheet.write(row, count, str(_(error)))
                row += 1
    ...
    def _field_type(self, lis, model, operation):
        new_lis = lis
        for i, l in lis.items():
            field = self.env['ir.model.fields'].search([('model_id','=',model.id),('name','=',i)])
            if not field:
                continue
            if operation == 'create' and field.required and not l:
                raise UserError(_("The field %s is required") % field.name)
